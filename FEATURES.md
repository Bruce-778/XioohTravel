...
#### 方法二：直接修改数据库（高级）
如果需要批量调整或更复杂的操作，可以直接修改 PostgreSQL / Supabase 数据库：
```bash
# 先运行非破坏性初始化，补齐缺失表 / 字段 / 索引
npm run init-db

# 或将这份 SQL 粘贴到 Supabase SQL Editor 执行
scripts/sql/supabase-safe-init.sql
```
...
如果知道订单号，可以在数据库中查询：
```sql
SELECT * FROM bookings WHERE id = '订单号';
```
...
### Q: 可以批量调整价格吗？
A: 目前需要在后台逐个订单调整。如需批量操作，可以直接操作 PostgreSQL / Supabase 数据库，建议先运行 `npm run init-db`。

### Q: 如何添加新的车型？
A: 使用 Supabase SQL Editor 或直接修改数据库：
```bash
npm run init-db
```

### Q: 如何添加新的价格规则？
A: 在数据库中插入 `pricing_rules` 记录，或通过后台“价格管理”页面添加。
...
1. 开发服务器是否正常运行
2. `.env` 文件配置是否正确
3. 数据库是否已初始化（运行过 `npm run init-db` 或已在 Supabase SQL Editor 执行安全 SQL）
