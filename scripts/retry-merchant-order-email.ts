...
function parseModeFromArgs(): RetryMode {
  ...
  if (!firstArg) {
    throw new Error(
      "Usage: npm run retry:merchant-order-email -- <BOOKING_ID>\n   or: npm run retry:merchant-order-email -- --all-unsent [limit]"
    );
  }
  ...
  const result = await db.query(
    `SELECT id
     ...
     WHERE stripe_payment_status = 'paid'
       AND merchant_order_email_sent_at IS NULL
...
async function main() {
  ...
  if (mode.kind === "single") {
    const result = await sendMerchantOrderNotificationIfNeeded(mode.bookingId);
  ...
  if (bookingIds.length === 0) {
    console.log("No unsent paid merchant order notifications found.");
    return;
  }
  ...
  const results: Array<{
    bookingId: string;
    ok: boolean;
    result?: Awaited<ReturnType<typeof sendMerchantOrderNotificationIfNeeded>>;
    error?: string;
  }> = [];

  for (const bookingId of bookingIds) {
    try {
      const result = await sendMerchantOrderNotificationIfNeeded(bookingId);
      results.push({
        bookingId,
        ok: true,
        result,
...
main().catch((error) => {
  console.error("Failed to retry merchant order email:", error);
  process.exitCode = 1;
});
