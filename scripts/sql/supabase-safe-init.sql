...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  stripe_payment_fee_jpy INTEGER,
  ...
  refund_fee_deducted_jpy INTEGER,
...
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS stripe_payment_fee_jpy INTEGER;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS stripe_balance_transaction_id TEXT;
...
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS stripe_refund_id TEXT;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS stripe_refund_status TEXT;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS refund_amount_jpy INTEGER;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS refund_fee_deducted_jpy INTEGER;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS refund_requested_at TIMESTAMPTZ;
ALTER TABLE IF EXISTS bookings ADD COLUMN IF NOT EXISTS refunded_at TIMESTAMPTZ;
...
-- Real production pricing is managed by scripts/seed.ts and
-- scripts/sync-real-pricing.ts. Do not insert demo pricing from init SQL.
...
BEGIN
  ...
  IF to_regclass('public.pricing_rule_overrides') IS NOT NULL THEN
    IF NOT EXISTS (
